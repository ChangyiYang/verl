...
class DeltaCheckpointEngine(NCCLCheckpointEngine):
    ...
    def _stream_flushes(self, flush_iter, first: bool, global_steps, tag: str) -> tuple[int, int, int]:
        ...
        if _prof:
            print(
                f"[delta-base-send-profile] v={global_steps} flushes={n} "
                f"produce(gather+diff+encode)={_produce:.3f}s publish(broadcast)={_publish:.3f}s",
                flush=True,
            )
        logger.info(
    ...
    async def update_weights_via_server(self, server_adapter, global_steps: int | None = None) -> None:
        ...
        while True:
            ...
            _t["dispatch"] += _now() - _t2
            applied += 1
            del pos, val_u8, val, spec_t
            if meta["is_last"]:
                break
        if _prof and engine is not None:
            print(
                f"[delta-recv-profile] v={global_steps} flushes={applied} "
                f"meta_wait={_t['meta_wait']:.3f}s recv(broadcast)={_t['recv']:.3f}s "
                f"dispatch(ipc+decode+apply)={_t['dispatch']:.3f}s",
                flush=True,
            )
...
class DeltaShardedCheckpointEngine(DeltaCheckpointEngine):
    ...
    async def send_weights(self, weights, global_steps=None):
        ...
        _t = {"diff": 0.0, "gather": 0.0, "pack": 0.0, "bcast": 0.0}
        ...
        batch_k = int(_os2.environ.get("VERL_DELTA_BATCH_GATHER", str(self.batch_gather)))
        ...
        if _prof:
            print(
                f"[delta-send-profile] v={global_steps} flushes={n_flushes} "
                f"diff={_t['diff']:.3f}s gather={_t['gather']:.3f}s "
                f"pack={_t['pack']:.3f}s bcast={_t['bcast']:.3f}s "
                f"total={_time.perf_counter() - _wall0:.3f}s",
                flush=True,
            )
        logger.info(
...
@CheckpointEngineRegistry.register("delta_sharded_mcore")
class DeltaMegatronShardedCheckpointEngine(DeltaCheckpointEngine):
    """Megatron sharded delta (design doc option 2): sparse mcore delta -> rank0 rebuild -> native bridge.

    Each actor rank byte-diffs only its local mcore shard and gathers only the changed
    ``(local-position, value)`` pairs to rank 0 (grouped per rank). rank 0 rebuilds each rank's shard
    as a NaN-sentinel tensor, runs the *native* megatron->HF ``merge`` + ``to_hf_format`` (handed in
    per weight as a ``reconstruct`` closure over the bridge), and reads the non-NaN entries as the HF
    delta. Because merge/convert are pure permutations, this is bit-identical to full-convert-then-diff
    (validated offline in ``_runs/phase0_bitident.py``). So the pin shards across ranks, the gather
    carries only the delta, and the wire + receiver are inherited from ``DeltaCheckpointEngine`` unchanged.

    Expects the sharded megatron generator ``get_per_tensor_param_shard()`` yielding
    ``(name, local_shard, meta)`` where ``meta`` carries: ``tp_size`` (1 for replicated params),
    ``tp_group``, ``shard_shape``, ``contributes``, and ``reconstruct(shard_list) -> [(hf_name, hf_tensor)]``.
    """

    SENTINEL = float("nan")
    _assemble_flush = DeltaShardedCheckpointEngine._assemble_flush  # identical indices-flush builder

    def __init__(self, *args, encoding: str = "indices", **kwargs) -> None:
        super().__init__(*args, encoding=encoding, **kwargs)
        self._shard_snap: dict[str, torch.Tensor] = {}  # name -> pinned-CPU snapshot of THIS rank's shard
        self._shard_seeded = False

    async def send_weights(self, weights, global_steps=None):
        # All actor ranks participate (gather-v is collective); only torch rank 0 rebuilds + broadcasts.
        # rank 0 accumulates the reconstructed HF deltas into bucket_size-sized flushes and streams each
        # as it fills (then frees it), bounding peak memory to ~2 buckets rather than the whole model.
        assert self.rank <= 0, "Trainer workers other than rank 0 should not send weights."
        is_r0 = self.is_master
        first = not self._shard_seeded
        bucket: list = []
        ...
        for name, local, meta in weights:
            tp_size = int(meta["tp_size"])
            tp_group = meta.get("tp_group")
            contributes = bool(meta.get("contributes", True))
            local = local.detach().contiguous()
            flat = local.view(-1)
            snap = self._shard_snap.get(name)
            if snap is None or snap.numel() != flat.numel():
                snap = torch.empty_like(flat, device="cpu", pin_memory=True)
            if contributes:
                base = _bitflip_like(flat) if first else snap.to(flat.device, non_blocking=True)
                idx, val = shard_delta_indices(flat, base, 0)  # LOCAL shard coords
            else:
                idx = torch.empty(0, dtype=torch.int64, device=flat.device)
                val = torch.empty(0, dtype=flat.dtype, device=flat.device)
            snap.copy_(flat, non_blocking=True)
            self._shard_snap[name] = snap

            grouped = gather_v_grouped_to_rank0(idx, val, group=tp_group)
            if not is_r0:
                continue

            # rebuild each rank's shard as a NaN-sentinel tensor (even TP split -> uniform shard_shape),
            # then let the NATIVE bridge do all placement + layout.
            shard_shape = tuple(meta["shard_shape"])
            shard_list = []
            for r in range(tp_size):
                gi, gv = grouped[r]
                buf = torch.full(shard_shape, self.SENTINEL, dtype=local.dtype, device=local.device)
                buf.view(-1)[gi] = gv
                shard_list.append(buf)
            for hf_name, hf_tensor in meta["reconstruct"](shard_list):
                ...
                if pos.numel() == 0:
                    continue
                vals = fl[pos]
                changed_elems += int(pos.numel())
                wire_bytes += int(pos.numel()) * (4 + vals.element_size())
                # Same oversized-entry slicing as the FSDP sharded path.
                for s in range(0, pos.numel(), self.MAX_ENTRY_ELEMS):
                    e = min(s + self.MAX_ENTRY_ELEMS, pos.numel())
                    bucket.append(
                        (hf_name, str(hf_tensor.dtype).replace("torch.", ""), list(hf_tensor.shape), pos[s:e], vals[s:e])
                    )
                    bucket_bytes += (e - s) * 8 + (e - s) * vals.element_size()
                    if bucket_bytes >= self.bucket_size:
                        _seal()
        ...
        logger.info(
            "delta-sharded-mcore send v=%s %s flushes=%d (streamed)",
            global_steps, "FULL" if first else "delta", n_flushes,
        )
