...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    async def update_weights(self, global_steps: int = None, mode: str = "auto"):
        ...
        if effective_mode != "naive":
            # The sharded delta engines diff each rank's local shard (no full all-gather),
            # so they consume the sharded param generator instead of the full-tensor one:
            # delta_sharded -> FSDP shards; delta_sharded_mcore -> megatron mcore shards.
            if effective_mode in ("delta_sharded", "delta_sharded_mcore"):
