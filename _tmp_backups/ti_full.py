...
class FSDPEngine(BaseEngine):
    ...
    def get_per_tensor_param(self, layered_summon=False, base_sync_done=False, **kwargs):
        ...
        # VERL_NCCL_NO_LOAD=1 (experiment): skip the whole-shard load/offload round trip
        # like the delta shard path does -- per-param .to(device).full_tensor() streams
        # H2D lazily instead. FSDP2 non-LoRA only.
        _skip_load = (
            bool(_os.environ.get("VERL_NCCL_NO_LOAD")) or _os.path.exists("/tmp/verl_nccl_no_load")
        ) and not hasattr(
            getattr(self.module, "_fsdp_wrapped_module", self.module), "peft_config"
        )
        ...
        if not self._uses_fsdp2_cpu_offload_policy and not _skip_load:
            load_fsdp_model_to_gpu(self.module)
        ...
        if self._is_offload_param and not _skip_load:
            offload_fsdp_model_to_cpu(self.module)
