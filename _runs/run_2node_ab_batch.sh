...
run_one() {
  ...
  if [ "$WMODE" = "delta_sharded" ]; then
    DELTA_ARGS=( actor_rollout_ref.rollout.checkpoint_engine.backend=delta_sharded
      ...
      "+actor_rollout_ref.rollout.checkpoint_engine.engine_kwargs.delta_sharded.batch_gather=${BATCH_K:-0}"
...
echo "======== A: batch_gather=0 (per-param) ========"
BATCH_K=0 run_one delta_sharded
echo "======== B: batch_gather=32 ========"
BATCH_K=32 run_one delta_sharded
