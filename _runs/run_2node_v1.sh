#!/usr/bin/env bash
# V1 separate_async trainer 2-node validation for the delta checkpoint-engine backends.
# Node0 = hybrid trainer (4 GPU), node1 = standalone rollout (4 GPU).
# Launch:  WMODE=delta_sharded srun --nodes=2 --ntasks-per-node=1 --gres=gpu:4 --cpus-per-task=32 \
#            --time=00:40:00 bash _runs/run_2node_v1.sh
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name=v1-delta trainer.experiment_name=${WMODE} \
    ...
    trainer.nnodes=1 trainer.n_gpus_per_node=$NGPU || true
