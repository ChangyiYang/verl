...
set -xeuo pipefail
# runs BOTH delta_sharded and nccl back-to-back inside ONE allocation (same node pair).
...
# profile toggles must be exported BEFORE ray start so workers inherit them
export VERL_SYNC_PROFILE=1 VERL_PROFILE_DELTA_SEND=1 VERL_PROFILE_NCCL_SEND=1
...
run_one() {
  ...
  echo "==================== RUN_ONE WMODE=$WMODE START ===================="
  ...
  echo "==================== RUN_ONE WMODE=$WMODE END ===================="
}

echo "PINNED PAIR: $(scontrol show hostnames "$SLURM_JOB_NODELIST" | tr '\n' ' ')"
rm -f /tmp/verl_nccl_no_load
echo "======== PHASE 1: delta (naive full-gather) ========"
run_one delta
echo "======== PHASE 2: delta_sharded ========"
run_one delta_sharded
echo "======== PHASE 3: nccl NO-LOAD (file flag on) ========"
touch /tmp/verl_nccl_no_load
run_one nccl
rm -f /tmp/verl_nccl_no_load
echo "======== PHASE 4: nccl current ========"
run_one nccl
