#!/usr/bin/env bash
# V1 separate_async 4-node validation: 2 trainer nodes (16 GPU FSDP2) + 2 rollout nodes (16 GPU).
# Launch:  WMODE=delta_sharded srun --nodes=4 --ntasks-per-node=1 --gres=gpu:8 --cpus-per-task=32 \
#            --time=01:00:00 bash _runs/run_4node_v1.sh
#   WMODE=nccl | delta | delta_sharded
#   CODE_DIR: verl checkout to run (default /home/changyi/verl_pr_v1 = clean PR worktree)
set -xeuo pipefail
WMODE="${WMODE:?set WMODE=nccl|delta|delta_sharded}"
CODE_DIR="${CODE_DIR:-/home/changyi/verl_pr_v1}"
...
mkdir -p "$RAY_TMPDIR"
# Triton JIT cache must be node-local: on NFS, concurrent workers hit
# "OSError: [Errno 116] Stale file handle" when loading compiled kernels.
...
TOTAL_NODES=$((NNODES_TRAIN + NNODES_ROLLOUT))
for i in $(seq 1 90); do
  ...
  [ "${ng:-0}" -ge $((NGPU*TOTAL_NODES)) ] && break
  sleep 3
done
...
# V1 separate_async: train_batch_size must equal ppo_mini_batch_size.
python3 -m verl.trainer.main_ppo \
    ...
    data.train_batch_size=16 data.shuffle=False \
    ...
    actor_rollout_ref.actor.fsdp_config.strategy=${STRATEGY:-fsdp2} \
    actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu=1 \
    actor_rollout_ref.ref.log_prob_micro_batch_size_per_gpu=2 \
    actor_rollout_ref.rollout.log_prob_micro_batch_size_per_gpu=2 \
    critic.strategy=${STRATEGY:-fsdp2} \
    actor_rollout_ref.actor.grad_clip=1.0 \
    actor_rollout_ref.model.use_remove_padding=${RMPAD:-True} \
    ...
    trainer.project_name=fournode-delta trainer.experiment_name=${EXP_NAME:-$WMODE} \
    trainer.val_before_train=False trainer.test_freq=-1 trainer.save_freq=-1 \
    trainer.total_epochs=1 trainer.total_training_steps=${STEPS:-6} \
    trainer.resume_mode=disable \
    trainer.nnodes=${NNODES_TRAIN} trainer.n_gpus_per_node=$NGPU || true
